...
def prepare_data_llm(config: dict) -> List[pd.DataFrame]:
    train, dev, test = prepare_data(
        ...
        search_filter_social_media = SEARCH_FILTER_SOCIAL_MEDIA,
    )
...
def run_model(config: dict):
    ...
    output_dir = os.path.join("output", "llm", experiment_name)
    os.makedirs(output_dir, exist_ok=True)

    fewshot_df, test = prepare_data_llm(config)
